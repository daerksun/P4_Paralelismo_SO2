...
/*Computes the multiplication of two square matrices: result = A * B
Input:	pointer to save the result
	pointer to matrix A
	pointer to matrix B
	size of the square matrices
Output: EXIT_FAILURE; EXIT_SUCCESS*/
{
	int element, cont, elementH, elementV, totalElem;
	long double sum;
	...
	{
		sum = 0;
		elementH = element / sizeMat;
		elementV = element % sizeMat;
		for(cont = 0; cont < sizeMat; cont++)
		{
			sum += *(A + elementH) * *(B + elementV);
			elementH ++;
			elementV += sizeMat;
		}
		//printf("%Lf, ", sum);
...
Output: EXIT_FAILURE; EXIT_SUCCESS*/
{
	...
	#pragma omp parallel for private(elementH, elementV, cont, sum)
	for(element = 0; element < totalElem; element++)
	{
		sum = 0;
		elementH = element / sizeMat;
